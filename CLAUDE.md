...
- **`sdk-adapter.ts`** — Anthropic `/v1/messages` ↔ SDK, one turn per request (Claude Code owns the tool loop). `translateRequest(body, npm)` builds the SDK call params (messages, tools, tool_choice, instructions) with `allowSystemInMessages: true`; inline `role:'system'` messages remain in their original conversation positions so volatile reminders do not invalidate the stable prompt prefix. On public-API OpenAI GPT-5.6+ routes, Anthropic `cache_control` blocks become explicit OpenAI cache breakpoints under implicit 30-minute caching. ChatGPT/Codex OAuth sends a hashed Claude session-derived `prompt_cache_key` and strips Claude Code's volatile `x-anthropic-billing-header` attribution from OpenAI instructions, but still omits `prompt_cache_options` and explicit breakpoints because those controls produced successful-but-empty OAuth responses during compatibility testing. Cache reads and GPT-5.6 cache writes map to Anthropic `cache_read_input_tokens` and `cache_creation_input_tokens`. `streamAnthropicResponse` maps the SDK event `stream` to Anthropic SSE and aborts after 120 seconds without an event; `generateAnthropicResponse` handles non-streaming. `thought_signature` round-trips: encoded into the Anthropic `tool_use.id` as `{id}::ts::{signature}` and decoded back into `providerOptions.google.thoughtSignature` (Gemini puts the signature on the tool-call parts, captured at `tool-input-start`). The SDK handles Gemini's strict `thought_signature` echo-back correctly — the reason a hand-rolled Gemini-native path used to be required.

**OpenAI OAuth WebSocket continuation** (`src/oauth/responses-websocket.ts`): all ChatGPT/Codex OAuth Responses models use a persistent WebSocket transport (models with `preferWebSockets` require it). Connections are partitioned by provider, OAuth account, upstream model, normalized effort, and hashed Claude session. Each completed response becomes a validated chain head after relay captures its exact text/tool/reasoning output; a partition can retain multiple divergent heads for rewinds and branches. The next request selects the longest exact-prefix head, uses `previous_response_id`, and sends only incremental input; message text and tool results match exactly, while function-call argument strings are compared as canonical JSON because Claude Code parses and the SDK reserializes them. Current instructions/tools/options are still forwarded on that create because Claude Code can change them during a normal tool loop. A new branch starts with full context and becomes another eligible head. Failures, aborts, idle/hard TTL expiry, and socket loss safely fall back to full context. Concurrent unrelated requests use isolated sockets, and `previous_response_not_found` retries once with full context before anything is emitted downstream. OAuth requires `store:false` (a live `store:true` probe returns HTTP 400); connection-local state is not durable across reconnects.
...
**Antigravity gateway** (`src/antigravity/`): `relay-ai agy` / `antigravity` / `antigravity-ide` run a local fake Cloud Code (Gemini internal) API server (`cloud-code-gateway.ts::startCloudCodeGateway`) that Antigravity talks to instead of Google's real backend. `request-adapter.ts::translateRequest` converts Cloud Code's `generateContent`/`streamGenerateContent` request shape (Gemini-style uppercase JSON Schema types, `functionCall`/`functionResponse` parts) into Vercel AI SDK params; `response-adapter.ts::formatCloudCodeChunk` converts the SDK's streamed output back into Cloud Code SSE chunks. Antigravity routes every MCP tool call through one generic `call_mcp_tool` wrapper function whose `Arguments` field has no fixed schema (it must accept any MCP tool's params) — Google's own Gemini reliably fills that field with a real object, but third-party models routed through relay-ai often stringify it instead (`"Arguments": "{}"` rather than `{}`), which Antigravity's MCP execution rejects as `"Invalid request parameters"` for **every** MCP tool call regardless of model strength or free/paid tier. `response-adapter.ts::normalizeFunctionCallArgs` un-stringifies any top-level tool-call argument that parses as valid JSON to an object/array before the chunk is sent back to Antigravity, fixing this. When debugging Antigravity issues, relay-ai's own `--trace` log truncates bodies to 500 chars and never logs tool-call arguments — Antigravity's own richer per-session record at `~/.gemini/antigravity-cli/brain/<session-uuid>/.system_generated/logs/transcript_full.jsonl` (JSONL, one record per step, `tool_calls` entries carry exact arguments) is the better source of truth. See `ANTIGRAVITY-DEBUG-SESSION.md` for the full debugging history of this and other Antigravity integration issues.
...
- `startProxyCatalog(routes, startingAliasId, debug, inferenceLogPath?)` — multi-route catalog proxy for switch-menu sessions; the optional log path records upstream response failures.
...
**Server tab (`relay-ai ui`):** Runs the same gateway as `relay-ai server`, in-process inside the `relay-ai ui` web server — no child process, no PID file. Stops automatically when the UI process exits.
...
- Persists the same settings the CLI wizard saves (`setServerExposedProviders`, `setServerMaskGatewayIds`, `setServerFavoritesOnly`, `setSavedServerPassword`) so a `relay-ai server` quick-start run in the terminal picks up whatever was last configured from the browser, and vice versa.
...
- Not every `modelFormat` is exposed via `relay-ai server`/`relay-ai ui` yet — `target-compatibility.ts` excludes formats from the `server` target that `server/router.ts` has no handler for (it only implements `'anthropic'` and `'openai'`).
